...
namespace Supercluster.KDTree
{
    ...
    [Serializable]
    public class KDTreePQ<TDimension, TNode>
        where TDimension : IComparable<TDimension>
    {
        ...
        {
            var nearestNeighborList = new PriorityQueue<int, double>(neighbors);
        ...
        /// <param name="nearestNeighbors">The <see cref="PriorityQueue{TElement, TPriority}{TElement,TPriority}"/> containing the nearest neighbors already discovered.</param>
        ...
        private void SearchForNearestNeighbors(
            ...
            int dimension,
            PriorityQueue<int, double> nearestNeighbors,
            double maxSearchRadiusSquared)
        {
            ...
            if (distanceSquaredToTarget.CompareTo(maxSearchRadiusSquared) <= 0)
            {
                nearestNeighbors.Enqueue(nodeIndex, distanceSquaredToTarget);
            }
        }
    }
